...
if len(arg)>0:
        if os.path.isfile(arg+".html"):
                print "Scraping posts from "+arg+"<hr>"
                with io.open(arg+".html", mode="r", encoding="utf-8") as fp:
...
for post in posts:
   ...
   try:
	...
	post_url='https://facebook.com'+str(post_url)
     ...
     shared_url=''
     try:
        tmp=re.search(r' shared a[n]? <a.+?href="(.+?)".*?>(.+?)</a>',str(post))
	...
	pass
     ...
     post_text=re.sub(' +',' ',re.sub('See Translation$', '',str(post_text)))
	
     if is_shared==1:
	post_text=post_text+" <a href='"+shared_url+"' target=_blank>"+shared_url+"</a>"	
     post_images=''
     post_videos=''
     vid_count=0
     pic_count=0
     ...
     try:
         post_pics=post.find_all('img', attrs={'class':regex3})
         for pic in post_pics:
	   if pic['src'] not in post_images:
            post_images=post_images+" "+pic['src']

         more_pics=post.find_all('img', attrs={'class':'_46-i img'})
	...
	pass
     ...
     link_count=0
     try:
         pst_links=post.find_all('a', attrs={'data-lynx-mode':regex4})
         for l in pst_links:
            tmp=re.search(r'facebook\.com\/l\.php\?u\=(http.+?)\&', l['href']).group(1)
            if (len(tmp)>0):
               l['href']=urllib.unquote(tmp)
	       if l['href'] not in post_links:
           	 post_links=post_links+" "+l['href']
	 urls = re.findall('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!<>*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', str(post_text))
	 for l in urls:
     ...
     try:
        post_likes_details=post.find('div', attrs={'class':'UFILikeSentenceText'}).text.encode('utf-8')
	...
	try:
           post_likes_details=str(post.find('div', attrs={'class':'_3t53 _4ar- _ipn'}))
	   indx=0
	   for react in reactions: 
	     try:
		tmp=re.search('"(\d+[.]?\d*[K]?[M]?) '+react+'"',post_likes_details).group()
		tmp=re.sub(r'"(\d+[.]?\d*[K]?[M]?) '+react+'"',r'\1',tmp)
		tmp=clean_num(tmp)
                if (int(tmp)>=0):
     ...
     try:
	  ...
	  #print "Error10"
        ...
        try:
          ...
          tmp=re.search(r'\d+[.]?\d*[K]?[M]?', post_comments).group()
	  tmp=clean_num(tmp)
	  ...
	  #print "Error11"
        ...
        try:
          ...
          tmp=re.search(r'\d+[.]?\d*[K]?[M]?', post_replies).group()
          tmp=clean_num(tmp)
          comment_count=comment_count+int(tmp)
        except:
	  #print "Error12"
	...
	#print "Error13"
     ...
     print "<tr><td><center>"+str(index)+"</center></td>"
     print "<td><a href='"+post_url+"' target=_blank>"+post_id+"</a></td>"
     print "<td>"+str(post_created_time)+"</td>"
     print "<td>"+str(profile_name)+"</td>"
     print "<td><a href='https://facebook.com/"+str(profile_id)+"' target=_blank>"+str(profile_id)+"</td>"
     print "<td>"+post_text+"</td>"
     print "<td><center>"+str(shared_as)+"</center></td>"
     print "<td><center>"+str(pic_count)+"</center></td>"
     if (pic_count>0):
         pic_elements=''
	 pictures=post_images.split(' ')
	 for pic in pictures:
           pic_elements=pic_elements+' <br><br> '+'<a href="'+pic+'" target=_blank><img src="'+pic+'" width=100></a>'
         print "<td>"+str(pic_elements)+"</td>"
     else:
	print "<td></td>"
     print "<td><center>"+str(vid_count)+"</center></td>"
     if (vid_count>0):
         vid_elements=''
         videos=post_videos.split(' ')
         for vid in videos:
           vid_elements=vid_elements+' <br><br> '+'<a href="'+post_url+'" target=_blank><img src="'+vid+'" width=100></a>'
         print "<td>"+str(vid_elements)+"</td>"
     else:
        print "<td></td>"
     print "<td><center>"+str(link_count)+"</center></td>"
     if (link_count>0):
         link_elements=''
         links=post_links.split(' ')
	 cnt=1
         for lnk in links:
           link_elements=link_elements+' '+'<a href="'+lnk+'" target=_blank>'+str(cnt)+'</a>'
	   cnt=cnt+1
         print "<td><center>"+str(link_elements)+"</center></td>"
     else:
        print "<td></td>"
     print "<td><center>"+str(post_reaction)+"</center></td>"

     indx=0
     for react in reactions:
        print "<td><center>"+str(post_reactions[indx])+"</center></td>"
        indx=indx+1

     print "<td><center>"+str(post_shares)+"</center></td>"
     print "<td><center>"+str(comment_count)+"</center></td></center>"
     print "</tr>"
