...
for arg in args:
   ...
   for post in posts:
	...
	output=output+"\n<tr><td><center>"+str(index)+"</center></td><td><a href='"+post_url+"' target=_blank>"+post_id+"</a></td><td>"+str(post_created_time)+"</td><td>"+str(profile_name)+"</td><td><a href='https://facebook.com/"+str(profile_id)+"' target=_blank>"+str(profile_id)+"</td><td>"+post_text+"</td><td><center>"+str(shared_as)+"</center></td><td><center>"+str(pic_count)+"</center></td>"
	if (pic_count>0):
	    pic_elements=''
	   ...
	   output=output+"<td></td>"
	output=output+"<td><center>"+str(vid_count)+"</center></td>"
	if (vid_count>0):
	    vid_elements=''
	   ...
	   output=output+"<td></td>"
	output=output+"<td><center>"+str(link_count)+"</center></td>"
	...
	if (link_count>0):
	    ...
	    for lnk in links:
	      ...
	      cnt=cnt+1
	    output=output+"<td><center>"+str(link_elements)+"</center></td>"
	else:
	   output=output+"<td></td>"
	output=output+"<td><center>"+str(post_reaction)+"</center></td>"

	indx=0
	for react in reactions:
	   output=output+"<td><center>"+str(post_reactions[indx])+"</center></td>"
	   indx=indx+1
   ...
   output=output+"</table><hr> Download CSV from <a href='"+arg+".csv"+"'>here</a></body></html>"

   if commandline:
	   with io.open(arg+"-scraped.html",'w',encoding='utf8') as f:
	       f.write(unicode(output, "utf-8"))
	   print "Saved view of scraped data to "+arg+"-scraped.html\n---\n"
   else:
	   print output
